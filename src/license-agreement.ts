...
window.onload = () => {
  var countrySelect = <HTMLSelectElement>document.getElementById('signature-country');

  var country: string;

  var signatureType: string = document.getElementById('signature-type').innerText;

  var viewerLang: string = document.getElementById('viewer-language').innerText;

  var countryForm: HTMLElement;

  class LicenseAgreement {
    readonly SVGSignatureData: string
    ...
    constructor(signature: string, type: string, lang: string) {
  ...
  function loadFormLayout() {
    ...
    Array.from(<HTMLCollectionOf<HTMLDivElement>>document.getElementsByClassName('signature-form')).forEach((form) => {
      form.hidden = true;
    });
  ...
  document.getElementById('signature-save').addEventListener('click', (e) => {
    ...
    Array.from(<HTMLCollectionOf<HTMLInputElement>>countryForm.getElementsByTagName('input')).forEach((input) => {
      license[input.name] = input.value;
    });
    license.Email = (<HTMLInputElement>document.getElementById('signature-email')).value;
    license.GitHubUsername = (<HTMLInputElement>document.getElementById('github-username')).value;
  ...
  function post(license: LicenseAgreement) {
    ...
    xhr.onreadystatechange = () => {
      if (xhr.readyState == XMLHttpRequest.DONE && xhr.status == 200) {
        ...
        Array.from(<HTMLCollectionOf<HTMLDivElement>>document.getElementsByClassName('shaded-form')).forEach((form) => {
          form.hidden = true;
        });
